...
**Evidencia:**
- ❌ No existe `/var/log/extractreq-post-boot.log`
...
ssh -i ~/.ssh/extractreq_terraform root@<IP_DEL_SERVIDOR>
...
cat /var/log/extractreq-post-boot.log
```
...
systemctl status extractreq-backend.service
```
...
journalctl -u extractreq-backend.service -n 50
```
...
```bash
ls -la /opt/extractreq-post-boot.sh
...
- [ ] El archivo `/var/log/extractreq-post-boot.log` existe
- [ ] El script `/opt/extractreq-post-boot.sh` NO existe (se auto-eliminó)
- [ ] El servicio `extractreq-backend.service` está activo
