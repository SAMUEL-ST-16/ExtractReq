...
```bash
# SSH key está en ~/.ssh/extractreq_terraform
ssh -i ~/.ssh/extractreq_terraform root@TU_IP_PUBLICA
```
...
```bash
# Backend
ssh root@TU_IP "sudo journalctl -u extractreq-backend -f"
