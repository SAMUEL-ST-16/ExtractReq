...
### Hostname
- Pon un nombre descriptivo: `extractreq-server`
...
1. **URL del repositorio Git**
   ```
   Ejemplo: https://github.com/tu-usuario/ExtractReq.git
   ```
...
5. **Nombre del modelo binario**
   ```
   Por defecto: SamuelSoto7/ExtractReqV8_Binario
   Presiona Enter para usar el valor por defecto
   ```

6. **Nombre del modelo multiclase**
   ```
   Por defecto: SamuelSoto7/ExtractReqV2_Multiclass
   Presiona Enter para usar el valor por defecto
   ```

7. **Dominio** (OPCIONAL)
   ```
   Si tienes un dominio (ej: extractreq.com), ingrésalo
...
3. Deberías ver el frontend de ExtractReq
...
sudo journalctl -u extractreq-backend -n 50
```
...
```bash
sudo nano /opt/extractreq/Backend/.env
```
...
```bash
# Desde tu computadora local
scp -r root@TU_IP:/opt/extractreq ./backup-extractreq
```
