# Guía de Despliegue - ExtractReq

Esta guía completa te ayudará a desplegar tu aplicación **ExtractReq** en producción.
...
Esto generará los archivos estáticos en la carpeta `dist/ExtractReq/browser/`.
...
serve -s dist/ExtractReq/browser -l 4200
```
...
COPY dist/ExtractReq/browser /usr/share/nginx/html
...
services:
  backend:
    ...
    container_name: extractreq-backend
    ports:
      - "8000:8000"
    env_file:
      - .env
  ...
  frontend:
    ...
    container_name: extractreq-frontend
    ports:
      - "80:80"
    depends_on:
      - backend
...
5. **Obtén la URL del backend**: https://extractreq-backend.railway.app
...
3. **Configurar:**
   ...
   - Output Directory: `dist/ExtractReq/browser`
4. **Variables de entorno**:
   - `API_URL`: URL de tu backend en Railway
5. **Deploy**

Vercel te dará una URL: https://extractreq.vercel.app
...
2. Configurar:
   ...
   - Publish Directory: `Frontend/dist/ExtractReq/browser`
3. Deploy
...
```bash
sudo nano /etc/systemd/system/extractreq-backend.service
```
...
Type=simple
User=tu-usuario
WorkingDirectory=/ruta/a/ExtractReq/Backend
Environment="PATH=/ruta/a/ExtractReq/Backend/venv/bin"
ExecStart=/ruta/a/ExtractReq/Backend/venv/bin/uvicorn app.main:app --host 127.0.0.1 --port 8000
Restart=always
RestartSec=10
...
sudo systemctl enable extractreq-backend
sudo systemctl start extractreq-backend
sudo systemctl status extractreq-backend
```
...
```bash
sudo nano /etc/nginx/sites-available/extractreq
```
...
server {
    ...
    # Frontend
    root /ruta/a/ExtractReq/Frontend/dist/ExtractReq/browser;
    index index.html;
...
```bash
sudo ln -s /etc/nginx/sites-available/extractreq /etc/nginx/sites-enabled/
sudo nginx -t
...
**Backend (systemd):**
```bash
sudo journalctl -u extractreq-backend -f
```
...
source venv/bin/activate
pip install -r requirements.txt
sudo systemctl restart extractreq-backend
```
...
Tu aplicación **ExtractReq** está ahora desplegada en producción.
...
**Desarrollado con ❤️ para análisis de requisitos de seguridad con IA**
