#!/bin/bash
#
# 🚀 ExtractReq - Despliegue Completamente Automatizado con Terraform
#
...
╔══════════════════════════════════════════════════════════╗
║                                                          ║
║   🚀 ExtractReq - Despliegue Automatizado con Terraform  ║
║                                                          ║
...
SSH_KEY_PATH="$HOME/.ssh/extractreq_terraform"

if [ ! -f "$SSH_KEY_PATH" ]; then
    echo "No se encontró clave SSH. Generando una nueva..."
    ssh-keygen -t rsa -b 4096 -f "$SSH_KEY_PATH" -N "" -C "extractreq-terraform"
...
echo "   Click en 'Generate New Token' → Nombre: 'ExtractReq' → Read & Write"
echo ""
...
echo "   Ejemplo: https://github.com/tu-usuario/ExtractReq.git"
echo ""
...
echo ""
read -p "Modelo binario [SamuelSoto7/ExtractReqV8_Binario]: " BINARY_MODEL
BINARY_MODEL=${BINARY_MODEL:-SamuelSoto7/ExtractReqV8_Binario}

read -p "Modelo multiclase [SamuelSoto7/ExtractReqV2_Multiclass]: " MULTICLASS_MODEL
...
echo -e "    ${CYAN}$SSH_COMMAND \"sudo journalctl -u extractreq-backend -f\"${NC}"
echo ""
