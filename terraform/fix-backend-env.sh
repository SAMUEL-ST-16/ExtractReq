#!/bin/bash
#
# Script de Arreglo Rápido del Backend - ExtractReq
...
echo "=========================================="
echo "  ExtractReq - Arreglo Rápido Backend"
echo "=========================================="
echo ""

echo "==> 1. Verificando archivo .env actual:"
cat /opt/extractreq/Backend/.env
echo ""

echo "==> 2. Creando backup del .env actual:"
cp /opt/extractreq/Backend/.env /opt/extractreq/Backend/.env.backup
echo "Backup guardado en: /opt/extractreq/Backend/.env.backup"
echo ""

echo "==> 3. Leyendo valores actuales:"
HF_TOKEN=$(grep HUGGINGFACE /opt/extractreq/Backend/.env | cut -d'=' -f2 || echo "")
BINARY_MODEL=$(grep BINARY_MODEL_NAME /opt/extractreq/Backend/.env | cut -d'=' -f2 || echo "SamuelSoto7/ExtractReqV8_Binario")
MULTICLASS_MODEL=$(grep MULTICLASS_MODEL_NAME /opt/extractreq/Backend/.env | cut -d'=' -f2 || echo "SamuelSoto7/ExtractReqV2_Multiclass")
GROQ_KEY=$(grep GROQ_API_KEY /opt/extractreq/Backend/.env | cut -d'=' -f2 || echo "")
OPENAI_KEY=$(grep OPENAI_API_KEY /opt/extractreq/Backend/.env | cut -d'=' -f2 || echo "")
...
chown extractreq:extractreq /opt/extractreq/Backend/.env
chmod 600 /opt/extractreq/Backend/.env
...
journalctl -u extractreq-backend -n 20 --no-pager
echo ""
...
echo "  journalctl -u extractreq-backend -n 100 --no-pager"
echo ""
