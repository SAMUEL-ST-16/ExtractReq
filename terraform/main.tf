...
resource "digitalocean_droplet" "extractreq" {
  # Nombre del servidor
  name   = "extractreq-server"
  ...
  # Tags para organizar
  tags = ["extractreq", "terraform", "production"]
...
resource "digitalocean_firewall" "extractreq" {
  name = "extractreq-firewall"

  droplet_ids = [digitalocean_droplet.extractreq.id]
...
# resource "digitalocean_domain" "extractreq" {
#   name       = var.domain_name
#   ip_address = digitalocean_droplet.extractreq.ipv4_address
# }
