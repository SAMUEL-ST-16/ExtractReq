"""
ExtractReq Backend - FastAPI Application
...
from app.core.exceptions import ExtractReqException
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("=" * 60)
    logger.info("Starting ExtractReq Backend API")
    logger.info("=" * 60)
    ...
    # Shutdown
    logger.info("Shutting down ExtractReq Backend API")
...
@app.exception_handler(ExtractReqException)
async def extractreq_exception_handler(request: Request, exc: ExtractReqException):
    """Handle custom ExtractReq exceptions"""
    logger.error(f"ExtractReq Exception: {exc.message}")
    return JSONResponse(
        status_code=500,
        content={
